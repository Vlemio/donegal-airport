...
- [ ] **Analíticas (GA4) sin configurar** — `GA4_MEASUREMENT_ID` en `BaseLayout.astro` sigue en `"REPLACE_WITH_REAL_GA4_ID"`. Google Consent Mode v2 + banner de cookies ya están montados y esperando; solo falta el ID real. **Al activarlo, configurar "eventos clave" desde el día 1** (marcar `form_submit` y los redirects de `/fly/aerlingus` · `/fly/loganair` como eventos clave en Admin → Eventos clave) — analizando el GA4 de la web actual (2026-09-10) se vio que en 8+ meses nunca se marcó ni un solo evento como clave, así que las secciones "Generar oportunidades de venta"/"Generar ventas" del panel llevan vacías todo este tiempo
...
- [ ] **Quitar el `noindex` antes del lanzamiento real** — `<meta name="robots" content="noindex, nofollow">` en `BaseLayout.astro` + `Disallow: /` en `public/robots.txt`, a propósito mientras vive en vercel.app. Recordatorio para el día que se mueva al dominio real (donegalairport.ie). **Mismo día, actualizar también:**
...
### Protección de `/tools/*` (2026-09-18)

- [x] ~~`/tools/image-position` sin ningún tipo de acceso~~ — resuelto 2026-09-18: `src/middleware.ts` exige HTTP Basic Auth en cualquier ruta bajo `/tools/` (hoy solo `image-position`, pero cubre futuras herramientas internas también). La página pasó de `prerender: true` a `false` — si no, el middleware nunca se ejecuta para ella (Vercel sirve las páginas prerenderizadas como HTML estático, sin pasar por la función servidor)
- [ ] **Configurar `TOOLS_AUTH_USER` / `TOOLS_AUTH_PASS` en Vercel** — el gate ya está en el código pero, sin estas dos variables de entorno puestas en Vercel (Settings → Environment Variables), se queda abierto sin contraseña (fallback intencional para no bloquear el dev local por accidente). Elegir usuario/contraseña y ponerlos ahí para que la protección sea real en producción

### Migración a Cloudflare + dominio real (2026-09-19)

Vercel es solo entorno de pruebas — el lanzamiento real será en **Cloudflare Workers** (Cloudflare ya no lo llama "Pages" en su integración con Astro) con el dominio `donegalairport.ie`, que hoy vive en la web vieja de WordPress (hosting Bigwetfish).

- [ ] **Pedir acceso a Google Search Console** de la cuenta que gestiona `donegalairport.ie` — necesario para: (1) ver qué webs externas enlazan a qué secciones concretas (Links → Top linking pages, para priorizar bien los redirects), y (2) enviar el sitemap nuevo y pedir reindexación el día del lanzamiento
- [x] ~~Averiguar quién controla el dominio~~ — resuelto 2026-09-19: WHOIS oficial (IEDR/weare.ie) confirma que el **registrador es BB Online** (bb-online.ie), no Bigwetfish — Bigwetfish solo gestiona DNS/hosting (nameservers `ns4/5/6.bigwetfish.co.uk`). Titular: el propio aeropuerto (no oculto por privacidad). No se sabe cuánto se paga actualmente por el dominio (dato de facturación privado, no visible en WHOIS) — preguntar directamente a BB Online o revisar si Bigwetfish lo factura como reseller
- [x] ~~Mapeo de URLs viejas → nuevas~~ — hecho 2026-09-19: 127 URLs reales sacadas del sitemap.xml de la web vieja (`donegalairport.ie/sitemap_index.xml`), mapeadas a su destino en la web nueva con nivel de confianza. Fichero en `docs/url-migration-map.csv` (~25 filas de confianza media/baja necesitan confirmación humana — páginas sin equivalente claro todavía, ej. "careers", "lost property", los 4 artículos duplicados de TG4)
